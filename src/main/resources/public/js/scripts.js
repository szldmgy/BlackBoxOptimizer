...
function addEmptyDependencyControl(paramdivid,paramname,depenedencyid, value,lower, upper, otherInfo, modifiable = true){
    $("#"+paramdivid).append("<div id = \"range_"+depenedencyid+"\" class = 'range'>\
                ...
                <input type=\"checkbox\"  name=\""+depenedencyid+"_value_float\" id = \""+depenedencyid+"_value_float\"  value =\""+value+"\" class='"+paramname+"_bool'required >\
...
function addAddDependencyButton(paramdivid,paramname, depenedencyid){
...
function showAddDependencyButton(paramdivid,paramname, depenedencyid){
    var id = "#"+ paramdivid+"_addDependencyButton";
...
function addRemoveRangeButton(paramdivid,paramname, depenedencyid){
    $("#range_"+depenedencyid).append("<button id = \""+depenedencyid+"_removeRangeButton\" type=\"button\" onclick=\"removeRange(\'"+paramdivid+"\',\'"+paramname+"\',\'"+depenedencyid+"\')\">Remove range</button>")
}
...
function addDependency(paramdivid, paramname, depenedencyid,deplower,depupper,modifiable = true){
        ...
        //document.getElementById(rangeid).appendChild(input2)
        var tn4 =  document.createTextNode("]")
        //document.getElementById(rangeid).appendChild(document.createTextNode("]"))
       ...
       // $( "#myselect option:selected" ).text();
        //todo why is this not working???
              // $("."+selected_param+"_string").on('change', function (e) {
       // $("."+selected_param+"_input_string").on('change',function () {testEnum(selected_param)})
       ...
       element.parentNode.removeChild(element);
        ...
        if(!modifiable){
            ...
            //document.getElementById(rangeid).innerHTML+=("<div id='"+ rangeid+ "_other_param_div'></div>")
            ...
            document.getElementById(divid).appendChild(input1_bool)
         ...
         if($("#"+paramdivid).children('range').length % 2 == 0){
               ...
               $(selector).on('change', function (e) {
                   ...
                   // input2_string.setAttribute("class",/* selected_param+"_input_string "+ */selected_param+"_string");
                   // input2_int.setAttribute("class", selected_param+"_int");
                   // input2_float.setAttribute("class", selected_param+"_float");
                   // input2_bool.setAttribute("class", selected_param+"_bool");

                   // modifyParamType(selected_param,type)
                    updateType(selected_param)
...
function paramNameEdited(name, id){
     ...
     $( ".paramselector option" ).each(function(){
           ...
           //var id = name_param_id_map.get(oldname)
            ...
            /* var newparamnames = []
             $(".paramname").each(function( index ) {
                 console.log( index + ": " + $( this ).val() );
                 newparamnames.push($( this ).val())
             });
             ...
             /*$("."+oldname+"_dep_lower_string").each(function(){$(this).attr('class', name+'_dep_lower_string')});
             $("."+oldname+"_dep_lower_int").each(function(){$(this).attr('class', name+'_dep_lower_int')});
             $("."+oldname+"_dep_lower_float").each(function(){$(this).attr('class', name+'_dep_lower_float')});
             $("."+oldname+"_dep_lower_bool").each(function(){$(this).attr('class', name+'_dep_lower_bool')});
             $("."+oldname+"_dep_upper_string").each(function(){$(this).attr('class', name+'_dep_upper_string')});
             $("."+oldname+"_dep_upper_int").each(function(){$(this).attr('class', name+'_dep_upper_int')});
             $("."+oldname+"_dep_upper_float").each(function(){$(this).attr('class', name+'_dep_upper_float')});
             $("."+oldname+"_dep_upper").each(function(){$(this).attr('class', name+'_dep_upper')});*/
...
/*function updateCommandLine(name, oldname){
    ...
    $("#"+commandlinetextinputid).val(actual_text.split(oldname).join(name))
}*/
function updateCommandLine(name, oldname){
    var actual_text = $("#"+commandlinetextinputid).val()
...
/*function loadParamPage(){
    ...
    $( "#setup_file_form" ).attr('method','post');
    $( "#setup_file_form" ).attr('action','/loadsetup');
    $( "#setup_file_form" ).submit();
    window.location.reload();
}
$("#setup_file_div").submit(function(event){
     window.location.reload();
});*/
...
function removeDependency(paramdivid, paramname, depenedencyid){
   ...
   // var z =
    ...
    //?????????
    //$("#dependency_ids").val(x.replace(depenedencyid,''));

    //$("#param_range_div_ids").val($("#param_range_div_ids").val().replace(depenedencyid,''));
    /*var element = document.getElementById(rangeid);
    //element.parentNode.removeChild(element);
    element = document.getElementById(rangeid+"_dep_lower");
        element.parentNode.removeChild(element);
    element = document.getElementById(rangeid+"_dep_upper");
    element.parentNode.removeChild(element);
    element = document.getElementById(rangeid+"_remove_dep_btn")
    element.parentNode.removeChild(element);
    element = document.getElementById(rangeid+"_select")
    element.parentNode.removeChild(element);*/


    var element = document.getElementById("range_"+depenedencyid+ "_other_param_div");
    element.parentNode.removeChild(element);
    addAddDependencyButton(paramdivid,paramname, depenedencyid)
...
function drawGraph(resultfilename ,obejectiveName,graphDiv){
    alert("Script loaded but not necessarily executed.");
      ...
      var xAxis = d3.svg.axis().scale(x)
       ...
       var valueline = d3.svg.line()
           ...
           .y(function(d) { return y(d[obejectiveName]); });
      ...
      // #set ($varName = "chart$obj")
       d3.csv(resultfilename, function(error, data) {
           if (error) throw error;
           data.forEach(function(d) {
               d[obejectiveName] = +d[obejectiveName];
        ...
        x.domain(d3.extent(data, function(d) { return d["iteration"]; }));
           y.domain([0, d3.max(data, function(d) { return d[obejectiveName]; })]);
           ...
           chart.selectAll("text").data(data).enter()
                  .append("text")
                   .text(function(d, i) { return d["iteration"]; })
                   .attr("y", 420)
                   .attr("x", function(d) { return x(d["iteration"]); })
                   .style("font-size", 10)
                   .style("font-family", "monospace");

                 g.selectAll(".value").data([data[data.length -1]]).enter()
                  .append("text")
                   .text(function(d, i) { return d[obejectiveName]; })
                   .attr("class", "value")
                   .attr("y", function(d) { return y(d[obejectiveName]); })
                   .attr("x", width - 20)
                   .style("font-size", 20)
                   .style("font-family", "monospace");
